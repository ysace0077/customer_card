...
class ConsultationCardManager {
    ...
    bindEvents() {
        ...
        document.getElementById('scanBtn').addEventListener('click', () => this.openDocumentScanner());
    ...
    openDocumentScanner() {
        if (!this.isMobileDevice()) {
            this.showMessage('문서 스캔은 모바일에서만 사용 가능합니다.', 'info');
            return;
        }

        // iOS Safari에서 문서 스캔 모드
        const fileInput = document.getElementById('cardImage');
        ...
        if (this.isIOSDevice()) {
            ...
            // Android에서 문서 스캔 앱 연동 시도
            fileInput.setAttribute('capture', 'environment');
            fileInput.setAttribute('accept', 'image/jpeg,image/jpg,application/pdf');
            
            this.showMessage('📄 문서 스캔 앱이 있다면 자동으로 열립니다.', 'info');
        }

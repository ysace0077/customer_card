class ConsultationCardManager {
    ...
    setupMobileOptimizations() {
        ...
        document.addEventListener('visibilitychange', () => {
            if (!document.hidden && this.githubConfig.owner && this.githubConfig.repo && this.githubConfig.token) {
                console.log('페이지 가시성 변경 - 자동 동기화 시작');
                setTimeout(() => {
                    this.syncData();
                }, 1000);
            }
    ...
    showSection(section) {
        ...
        if (section === 'search') {
            // 모바일에서 데이터 로딩 확인
            console.log('검색 탭 활성화, 카드 수:', this.cards.length);
            this.loadAllCards();
    ...
    async syncData() {
        ...
        try {
            ...
            // 2단계: GitHub에서 최신 데이터 가져오기 (캐시 무시)
            await this.loadCardsFromGitHub(true); // 강제 새로고침
            const remoteCount = this.cards.length;
            
            console.log(`동기화: 로컬 ${localCount}개, 원격 ${remoteCount}개`);
            
            // 3단계: 데이터 병합 (로컬이 더 최신이면 업로드)
            let needsUpload = false;
            
            if (localCount > remoteCount) {
                // 로컬에 더 많은 데이터가 있으면 병합
                const mergedCards = this.mergeCardData(localCards, this.cards);
                this.cards = mergedCards;
                needsUpload = true;
                console.log('로컬 데이터가 더 최신입니다. 업로드합니다.');
            } else if (localCount < remoteCount) {
                console.log('원격 데이터가 더 최신입니다.');
            } else {
                // 개수가 같으면 최신 업로드 날짜 비교
                const localLatest = this.getLatestUploadDate(localCards);
                const remoteLatest = this.getLatestUploadDate(this.cards);
            ...
            if (needsUpload) {
                await this.saveCardsToGitHub();
                this.showMessage(`동기화 완료: ${this.cards.length}개 카드 (업로드됨)`, 'success');
            } else {
                this.showMessage(`동기화 완료: ${this.cards.length}개 카드 (다운로드됨)`, 'success');
            }

class ConsultationCardManager {
    ...
    showSalespersonStats() {
        const statsContainer = document.getElementById('salespersonStats');
        if (!statsContainer) {
            // 통계 컨테이너가 없으면 생성
            const searchSection = document.getElementById('searchSection');
            const statsHtml = `
                <div id="salespersonStats" class="stats-section">
                    <h3 class="stats-title">📊 판매자별 상담 현황</h3>
                    <div class="stats-grid" id="statsGrid"></div>
                </div>
            `;
            searchSection.insertAdjacentHTML('afterbegin', statsHtml);
        }

        // 판매자별 통계 계산
        const salespersons = ['이호만 점장', '김승진 실장', '오희정 실장'];
        const stats = salespersons.map(person => {
            const personCards = this.cards.filter(card => card.salesperson === person);
            const thisMonth = new Date();
            const monthStart = new Date(thisMonth.getFullYear(), thisMonth.getMonth(), 1);
            const monthCards = personCards.filter(card => 
                new Date(card.consultationDate) >= monthStart
            );

            return {
                name: person,
                total: personCards.length,
                thisMonth: monthCards.length
            };
        });
            ...
            <div class="stat-card">
                ...
                <div class="stat-numbers">
                    ...
                    <div class="stat-month">
                        ...
                        <span class="stat-value highlight">${stat.thisMonth}</span>
                    </div>
                </div>
            </div>
        `).join('');
